...
for person in my_dataset:
    ...
    if msg_from_poi != "NaN" and to_msg != "NaN":
        my_dataset[person]['msg_from_poi_prop'] = msg_from_poi / float(to_msg)
    else:
        my_dataset[person]['msg_from_poi_prop'] = 0
    ...
    if msg_to_poi != "NaN" and from_msg != "NaN":
        my_dataset[person]['msg_to_poi_prop'] = msg_to_poi / float(from_msg)
    else:
        my_dataset[person]['msg_to_poi_prop'] = 0
new_features_list = features_list + ['msg_to_poi_prop', 'msg_from_poi_prop']
...
data = featureFormat(my_dataset, optimized_features_list +
                     ['msg_to_poi_prop', 'msg_from_poi_prop'],
                     sort_keys=True)
...
def validate_clf(grid_search, features, labels, params, iters=100):
    ...
    for i in range(iters):
        ...
        pre = pre + [precision_score(labels_test, predictions, average='macro')]
        recall = recall + [recall_score(labels_test, predictions, average='macro')]
    print "Accuracy Score: {}".format(mean(acc))
    print "Precision Score: {}".format(mean(pre))
    print "Recall Score:    {}".format(mean(recall))
...
validate_clf(nb_grid_search, new_f_features, new_f_labels, nb_param)
...
validate_clf(k_grid_search, features, labels, k_param)

print("Evaluate k-mean model with new features")
validate_clf(k_grid_search, new_f_features, new_f_labels, k_param)
...
validate_clf(lo_grid_search, features, labels, lo_param)

print("Evaluate logistic regression model using dataset with new features")
validate_clf(lo_grid_search, new_f_features, new_f_labels, lo_param)
...
validate_clf(s_grid_search, features, labels, s_param)

print("Evaluate SVM model using dataset with new features")
validate_clf(s_grid_search, new_f_features, new_f_labels, s_param)
...
pipeline = Pipeline(steps=[("SKB", selector), ("NaiveBayes", clf_gnb)])

grid = GridSearchCV(pipeline, nb_param, verbose=0, scoring='f1')
...
print "Accuracy of tuned GaussianNB CLF  : ", accuracy_score(labels_test, prediction)
print "Precision of tuned GaussianNB CLF : ", precision_score(prediction, labels_test)
print "Recall of tuned GaussianNB CLF    : ", recall_score(prediction, labels_test)
print "f1-score of tuned GaussianNB CLF        : ", f1_score(prediction, labels_test)
